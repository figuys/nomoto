...
public class DeviceRestoreWorkerEx : IDisposable
{
	...
	public Action<string, string, string, bool> ResourceItemFinishRestoreCallback { get; set; }
	...
	public void DoProcess(object state)
	{
		foreach (IWorker worker in _workerList)
		{
			...
			{
				w.ItemFinishRestoreCallback = delegate(string id, string path, bool isSuccess)
				{

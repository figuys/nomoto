...
public class DevicePicManagementV7 : DeviceCommonManagement, IDevicePicManagement
{
	...
	public List<ServerPicInfo> GetPicInfoList(string alblumId, string groupKey)
	{
		...
		long sequence = HostProxy.Sequence.New();
		if (tcpAndroidDevice.ConnectedAppType == "Ma")
		{
			if (messageReaderAndWriter.SendAndReceiveSync("getPICIdList", "getPICIdListResponse", new List<string> { alblumId, groupKey }, sequence, out List<ServerPicInfo> receiveData) && receiveData != null)
			{
				return receiveData;
			}
			return null;
		}
		if (messageReaderAndWriter.SendAndReceiveSync("getPICIdList", "getPICIdListResponse", new List<string> { alblumId, groupKey }, sequence, out List<string> receiveData2) && receiveData2 != null)
		{
			List<ServerPicInfo> picArr = new List<ServerPicInfo>();
			receiveData2.ForEach(delegate(string p)
			{
				picArr.Add(new ServerPicInfo
				{
					Id = p
				});
			});

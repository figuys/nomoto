...
public class PicOriginalHolderViewModel : ViewModelBase
{
	private PicInfoViewModel curPicInfoVM;

	private ObservableCollection<PicInfoListViewModel> groupArr;

	private readonly PicExportWorker m_picExportWorker;
	...
	public PicOriginalHolderViewModel(ObservableCollection<PicInfoListViewModel> picList, PicInfoViewModel vm)
	{
		...
		m_picExportWorker = new PicExportWorker(ExportImgResultHandler);
	}

	private PicInfoViewModel GetPicInfoVM(bool isNext = true, bool isDelete = false)
	{
	...
	public void ShowPic(PicInfoViewModel pic)
	{
		curPicInfoVM = pic;
	...
	private void DeletePicCommandHandler(object parameter)
	{
		...
		LenovoPopupWindow lenovoPopupWindow = new OkCancelWindowModel().CreateWindow(HostProxy.Host.HostMainWindowHandle, "K0585", "K0562", "K0208", "K0583", new BitmapImage(new Uri("Pack://application:,,,/lenovo.mbg.service.lmsa.phoneManager;Component/Assets/Images/PicPopup/delete.png")));
		lenovoPopupWindow.Owner = win;
		lenovoPopupWindow.WindowStartupLocation = WindowStartupLocation.CenterOwner;
		lenovoPopupWindow.ShowDialog();
		if (!lenovoPopupWindow.WindowModel.GetViewModel<OKCancelViewModel>().IsOKResult || !new DeviceCommonManagement().DeleteDevFilesWithConfirmEx(win, "deletePicturesById", new List<string> { curPicInfoVM.RawPicInfo.Id }))
		{
			return;
		}
		PicInfoViewModel picInfoVM = GetPicInfoVM(isNext: false, isDelete: true);
		if (picInfoVM == null)
		{
	...
	{
		PicInfoViewModel picInfoVM = GetPicInfoVM(isNext: false);
		if (picInfoVM != null)
		{
			ShowPic(picInfoVM);
		}
	...
	{
		PicInfoViewModel picInfoVM = GetPicInfoVM();
		if (picInfoVM != null)
		{
			ShowPic(picInfoVM);
		}
	...
	private void SetPicButtonEnable(PicInfoViewModel vm)
	{
		for (int i = 0; i < groupArr.Count; i++)
		{
			for (int j = 0; j < groupArr[i].Pics.Count; j++)

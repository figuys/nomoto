...
public class GifMakerViewModelV6 : ViewModelBase
{
	...
	public GifMakerViewModelV6(GifMakerViewV6 view)
	{
		...
		wnd.CommandBindings.Add(new CommandBinding(ImagePlotCmd, delegate(object sender, ExecutedRoutedEventArgs e)
		{
			...
			plotViewV.Title = LangTranslation.Translate("K0200");
			plotViewV.ViewModel.Model.DisplayImage = displayImage;
			plotViewV.ViewModel.Model.TempDir = Model.TempDir;
			plotViewV.ShowInTaskbar = false;
			plotViewV.WindowStartupLocation = WindowStartupLocation.CenterOwner;
			string uid = Guid.NewGuid().ToString("N");
			IntPtr owner = HostProxy.HostOperationService.ShowMaskLayer(uid);
			new WindowInteropHelper(plotViewV).Owner = owner;
			if (true == plotViewV.ShowDialog())
			{
				Model.IsAnimation = false;
				Model.IsChanged = true;
				imageEntitiy2.FilePath = plotViewV.ViewModel.Model.NewFile;
			}
	...
	private void ImageToGif(string giffile, int width, int height, bool repeat = true)
	{
		...
		ToolboxViewContext.SingleInstance.MessageBox.ShowMessage(prg);
	}

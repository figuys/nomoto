...
public partial class RingtoneMakerViewV6 : System.Windows.Controls.UserControl, IDisposable, IComponentConnector
{
	...
	private void SaveToDevice(string file)
	{
		...
		BusinessData businessData = new BusinessData(BusinessType.RINGTONE_MAKER_SAVE2DEVICE, currentDevice);
	...
	private TransferResult SaveAndSetRingtone(string fileName, string actionName, string resultKey, MusicType type)
	{
		...
		if (actionName == "setMusicAsRingtone")
		{
			...
			if (deviceMusicManagement.SetMusicAsRingtone(musicInfo, type) != 0)
			{
				return TransferResult.FAILD;
			}
			return TransferResult.SUCCESS;
	...
	private bool ValidateMusicEditOk()
	{
		...
		{
			ToolboxViewContext.SingleInstance.MessageBox.ShowMessage("K0071", "K0240");
			return false;
		}
		return true;
	}
